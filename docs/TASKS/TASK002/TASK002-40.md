...
阅读 `docs/Requirements/Core.md` 和 `docs/Features/Core.md` 文档，了解弹幕管理的详细需求：
- 弹幕管理：弹幕添加、弹幕修改、弹幕删除、弹幕审核
- 弹幕属于用户，需要关联用户表
- 弹幕支持视频分集和漫画章节两种内容类型
- 弹幕需要支持时间轴位置、弹幕内容、弹幕类型、弹幕颜色等
- 弹幕显示方式：
  - 视频弹幕：滚动弹幕（scroll）从右到左滚动，持续时间由滚动速度决定；顶部弹幕（top）和底部弹幕（bottom）固定显示，需要持续时间字段
  - 漫画弹幕：指定到特定位置显示，可能需要持续时间或固定显示
...
CREATE TABLE danmaku (
    ...
    CONSTRAINT ck_danmaku_duration CHECK (duration IS NULL OR (duration > 0 AND danmaku_type IN ('top', 'bottom'))),
...
COMMENT ON COLUMN danmaku.duration IS '持续时间，单位：秒，精确到毫秒。对于滚动弹幕（scroll）通常为NULL，由前端根据滚动速度自动计算；对于顶部弹幕（top）和底部弹幕（bottom）应设置持续时间，指定显示多长时间后消失；对于漫画弹幕，可为NULL（固定显示）或设置持续时间';
...
- **检查约束**：`ck_danmaku_duration` (duration, danmaku_type) - 确保持续时间大于0（如果存在），且持续时间仅在顶部弹幕（top）和底部弹幕（bottom）时设置，滚动弹幕（scroll）不应设置持续时间
...
- **可选字段**：duration 可为 NULL（滚动弹幕由前端自动计算，漫画弹幕可固定显示）
...
3. 外键约束正确，关联 users 表
4. 检查约束正确，确保内容类型、弹幕类型、时间轴位置、字体大小、持续时间、弹幕状态等是有效的值
5. 持续时间字段设计正确：滚动弹幕（scroll）通常为NULL，由前端根据滚动速度自动计算；顶部弹幕（top）和底部弹幕（bottom）应设置持续时间；漫画弹幕可为NULL或设置持续时间
6. 索引设计合理，支持常用查询场景（包括按内容查询弹幕、按用户查询弹幕、按时间轴位置排序）
7. 表注释和字段注释完整，使用中文，明确说明持续时间的使用规则
8. 触发器正确创建，自动更新 updated_at 字段
9. 表设计符合 PostgreSQL 数据库设计规范
10. 支持视频分集和漫画章节两种内容类型的弹幕
11. 支持弹幕类型（滚动、顶部、底部）、弹幕颜色、字体大小、持续时间等属性
