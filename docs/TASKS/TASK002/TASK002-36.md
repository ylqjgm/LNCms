...
阅读 `docs/Features/Core.md` 和 `docs/Requirements/Core.md` 文档，了解退费管理的详细需求：
- 退费管理：退费申请、退费审核、退费处理、退费状态管理
- 退费关联流水表，记录退费对应的原始流水
- 退费需要支持退费原因、退费金额、退费状态等
- 退费需要同时记录实际现金金额和虚拟货币替代金额
...
CREATE TABLE refunds (
    ...
    CONSTRAINT ck_refunds_virtual_currency_amount CHECK (virtual_currency_amount >= 0),
...
COMMENT ON COLUMN refunds.refund_amount IS '退费金额，实际现金金额，单位：元，必须大于0';
COMMENT ON COLUMN refunds.virtual_currency_amount IS '虚拟货币替代金额，虚拟货币单位，用于记录退费对应的虚拟货币金额，必须大于等于0，默认为0.00';
...
- **检查约束**：
  - `ck_refunds_refund_amount` (refund_amount) - 确保退费金额（实际现金金额）大于0
  - `ck_refunds_virtual_currency_amount` (virtual_currency_amount) - 确保虚拟货币金额大于等于0
  - `ck_refunds_refund_status` (refund_status) - 确保退费状态是预定义的值
- **非空约束**：transaction_id、refund_amount、virtual_currency_amount、refund_status 必须非空
- **默认值**：virtual_currency_amount 默认为 0.00，refund_status 默认为 'pending'
...
1. 表结构设计完整，包含所有必要字段（包括实际现金金额和虚拟货币金额）
2. 所有字段都有适当的类型、约束和默认值
3. 外键约束正确，关联 transactions 表和 users 表
4. 检查约束正确，确保退费金额（实际现金金额）大于0，虚拟货币金额大于等于0，退费状态是预定义的值
5. 退费金额字段区分实际现金金额（refund_amount，单位：元）和虚拟货币金额（virtual_currency_amount，虚拟货币单位）
6. 索引设计合理，支持常用查询场景（包括按流水查询、按状态查询、按时间排序）
7. 表注释和字段注释完整，使用中文
...
- `docs/Features/Core.md` - 核心模块功能概述文档（8.4 退费管理）
