...
在 `internal/api/v1/dto/config_dto.go` 文件中定义添加站群节点请求结构体：

```go
package dto

// AddClusterNodeRequest 添加站群节点请求
type AddClusterNodeRequest struct {
    NodeID     string `json:"node_id" binding:"required"`                       // 节点标识，必填，唯一
    NodeURL    string `json:"node_url" binding:"required,url"`                  // 节点地址，必填，URL格式
    NodeStatus string `json:"node_status" binding:"omitempty,oneof=active inactive"` // 节点状态，可选，默认active
    NodeWeight int    `json:"node_weight" binding:"omitempty,min=1,max=100"`    // 节点权重，可选，范围1-100，默认50
}

// ClusterNodeResponse 集群节点响应
...
### 2. 实现AddClusterNode方法

在ConfigHandler中实现AddClusterNode方法：

```go
// AddClusterNode 添加站群节点
// @Summary 添加站群节点
// @Description 添加集群节点，需要管理员权限
// @Tags 系统配置
// @Accept json
// @Produce json
// @Param request body dto.AddClusterNodeRequest true "添加站群节点请求"
...
// @Router /api/v1/system/config/cluster/nodes [post]
// @Security BearerAuth
func (h *ConfigHandler) AddClusterNode(c *gin.Context) {
    ...
    // 绑定请求参数
    var req dto.AddClusterNodeRequest
    ...
    for _, nodeInterface := range clusterNodes {
        ...
        if existingNodeID, ok := nodeMap["node_id"].(string); ok && existingNodeID == req.NodeID {
            response.Error(c, http.StatusBadRequest, "VALIDATION_ERROR", fmt.Sprintf("节点标识 %s 已存在", req.NodeID), nil)
            return
        }
    }
    ...
    newNode := map[string]interface{}{
        ...
        "node_weight": req.NodeWeight,
    }
    clusterNodes = append(clusterNodes, newNode)
    ...
    if err := h.configService.UpdateConfig(ctx, string(model.ConfigTypeCluster), configMap); err != nil {
        response.Error(c, http.StatusInternalServerError, "CONFIG_UPDATE_FAILED", "添加站群节点失败", err)
        return
    }
...
func RegisterConfigRoutes(r *gin.RouterGroup, configHandler *handler.ConfigHandler) {
    config := r.Group("/system/config")
    {
        ...
        {
            clusterNodes.POST("", configHandler.AddClusterNode) // 添加POST路由
        }
    }
}
```
...
1. **API端点正确实现**
   - POST /api/v1/system/config/cluster/nodes 端点正确实现
   - 路由正确注册
   - 权限验证中间件正确配置

2. **请求参数验证正确**
   - 请求参数绑定验证正确
   - 业务规则验证正确（节点标识唯一性、节点权重范围等）
   - 验证失败时返回适当的错误响应
